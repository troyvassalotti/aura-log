...
export class HistogramChart extends Base {
	 ...
	 */
	get #values() {
		return Object.values(this._data).map((value) => new Date(value.date).getHours());
	}
	...
	get chartOptions() {
		return {
			caption: {
				text: `Worst Time of the Day? Between ${this.#topHour}:00-${this.#topHour + 1}:00
        ${this.#topHour + 1 >= 12 ? "p.m." : "a.m."}`,
			...
			plotOptions: {
				...
				scatter: {
					tooltip: {
						pointFormat: "Headache No. <b>{point.x}</b><br/>Hour: <b>{point.y}</b>",
					},
				},
			},
			series: [

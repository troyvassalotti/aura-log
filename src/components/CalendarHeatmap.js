...
export class CalendarHeatmap extends Base {
	...
	#chooseTheme(theme) {
		switch (theme) {
			case "orange":
				return ["#864313", "#b3591a", "#df7020", "#e58c4d", "#e58c4d", "#f2c6a6"];
			case "blue":
				return ["#136086", "#1a7fb3", "#209fdf", "#4db2e5", "#79c6ec", "#a6d9f2"];
			case "green":
				return ["#138626", "#1ab333", "#20df40", "#4de566", "#79ec8c", "#a6f2b2"];
			case "purple":
				return ["#861386", "#b31ab3", "#df20df", "#e54de5", "#ec79ec", "#f2a6f2"];
		}
	}
	...
	#createHeatmap(data) {
		...
		svg
			...
			.selectAll("rect")
			.data((dataValue) => d3.timeDays(new Date(dataValue, 0, 1), new Date(dataValue + 1, 0, 1)))
			.enter()
			.append("rect")
			.attr("class", "day")
			...
			.attr(
				"x",
				(dataValue) => d3.timeWeek.count(d3.timeYear(dataValue), dataValue) * theme.cellSize,
		...
		svg
			...
			.selectAll("path")
			.data((dataValue) => d3.timeMonths(new Date(dataValue, 0, 1), new Date(dataValue + 1, 0, 1)))
			.enter()
			.append("path")
			.attr("class", "month")
			.attr("d", function(dataValue) {
				const t1 = new Date(dataValue.getFullYear(), dataValue.getMonth() + 1, 0),
					d0 = dataValue.getDay(),
					w0 = d3.timeWeek.count(d3.timeYear(dataValue), dataValue),
					d1 = t1.getDay(),
					w1 = d3.timeWeek.count(d3.timeYear(t1), t1);
				return `M${(w0 + 1) * theme.cellSize}, ${d0 * theme.cellSize}H${w0 * theme.cellSize}V${
					7 * theme.cellSize
				}H${w1 * theme.cellSize}V${(d1 + 1) * theme.cellSize}H${(w1 + 1) * theme.cellSize}V0H${
					(w0 + 1) * theme.cellSize
				}Z`;
			});
			...
			if (day.hasAttribute("fill")) {
				d3.select(day)
					.on("mouseover", (evt) => {
						...
						const left = evt.pageX > (window.innerWidth - rect.width)
